...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        Collect your 20 most recent friends (5 times).
      </td>
      <td>
...
That last example might seem contrived ("Why would I call `#take` 5 times?")
but it applies to any [`Enumerable`][enumerable] method you might call on a
cursor, including: `#all?`, `#any?`, `#collect`, `#count`, `#detect`, `#each`,
`#include?`, `#inject`, `#max`, `#min`, `#reject`, `#reverse_each`, `#select`,
`#sort`, `#sort_by`, `#take` and `#to_a`. In version 4, each time you called
one of those methods, it would perform *n* HTTP requests. In version 5, it only
will perform those HTTP requests the first time one of those methods is called.
...
* `#collection` and its aliases have all been removed.
